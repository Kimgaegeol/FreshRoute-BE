...
router.post("/", checkLoginMiddleware, async (req, res, next) => {
  try {
    // 1) 세션에서 account_idx 가져오기
    const account_idx = req.session.user.idx;

    // 2) 요청 바디에서 production_idx 추출
    const { production_idx } = req.body;
    if (!production_idx) {
      return res
        .status(400)
        .json({ success: false, message: "production_idx를 보내주세요." });
    }

    // 3) DB에 삽입
    const { rows } = await client.query(insertBucketSql, [
      account_idx,
      parseInt(production_idx, 10),
    ]);
    const bucket = rows[0];
...
/* DELETE /bucket/:bucket_idx — 로그인된 유저의 버킷 항목 삭제 */
router.delete("/:bucket_idx", checkLoginMiddleware, async (req, res, next) => {
  try {
    ...
    if (isNaN(bucket_idx)) {
      return res
        .status(400)
        .json({ success: false, message: "올바른 bucket_idx를 전달해주세요." });
    }

    const { rows } = await client.query(deleteBucketSql, [
      bucket_idx,
      account_idx,
    ]);
...
/* GET /bucket/list — 로그인된 유저의 버킷(장바구니) 항목 조회 */
router.get("/list", checkLoginMiddleware, async (req, res, next) => {
  try {
    const account_idx = req.session.user.idx;

    const { rows } = await client.query(listBucketSql, [account_idx]);
